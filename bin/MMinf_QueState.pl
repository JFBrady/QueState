...
Computes MMinf (Poisson) Dist QueState Probabilities (Calc 0-k) and Service Levels
...
##########################################################################
# Perl Script:                                                           #
# Purpose: Calculates MMinf QueState Probabilities and Service Levels    #
...
my($p_sum)=0;
my($psum_c_minus_1)=0;
my($dir_results)='MMinf_QueState_Results';
my($dir_traces)='MMinf_QueState_Traces';
...
p_term
{
  ...
  for($n=1;$n<=$k;$n++)
  {
    $term1 = $term1*($lamda/$mu);
    $nn = $nn*$n;
    $term2 = $term1/$nn;
    $pstate[$n] = $pstate[0]*$term2;
  }
  return(\@pstate);
}
...
out_record
{
  ...
  # Results Data  #
  #################
  $row = "\nMMinf QueState Results: $daydatetime";
  push @out_record,$row;

  ######################################
  # Performance Statistics             #
  ######################################
  $row = join ',','c','p[show]','lamda','mu','a','rho',
                  'p[block]','p_sum';
  push @out_record,$row;
  $row = join ',',$c,$k,$lamda,$mu,$a,$rho,
                  $pblock,$p_sum;
  push @out_record,$row;
...
trace
{
  ...
  # Trace Date #
  ##############
  $row = "\nMMinf QueState Trace Data: $daydatetime";
  push @trace,$row;

  ######################################
  # Input Parameters                   #
  ######################################
  $row = join ' ',"c=$c","k=$k","lamda=$lamda","mu=$mu","rho=$rho";
  push @trace,$row;
